...
void Session::Update(inc_pack* InPack,out_pack* OuPack)
{
    switch(InPack->gc())
    {
    case 0: break;
    case 0x003B: handle_smsg_char_enum(InPack,OuPack); return;           //SMSG_CHAR_ENUM
    case 0x0051: handle_smsg_name_query_response(InPack); return;        //SMSG_NAME_QUERY_RESPONSE
    case 0x0096: handle_smsg_messagechat(InPack,OuPack); return;         //SMSG_MESSAGECHAT
    case 0x0099: handle_smsg_channel_notify(InPack,OuPack); return;      //SMSG_CHANNEL_NOTIFY
    ...
    if (UnkPlayers.begin() != UnkPlayers.end())
        send_cmsg_name_query(OuPack,*UnkPlayers.begin());
}
